...
namespace IDCardNo
{
    public partial class IDCardHelper
    {
        ...
        public static IDCardInfo Parse(string id)
        {
            ...
            {
                County = getDivisionCodeData(region_code),
                Prefecture = getDivisionCodeData(region_code.Substring(0, 4)),
                Province = getDivisionCodeData(region_code.Substring(0, 2)),
                Birthday = DateTime.ParseExact(str_birthday, "yyyyMMdd", null),
                Gender = gender_code % 2 == 0 ? 0 : 1
            };
        }
        public static string getDivisionCodeData(string region_code)
        {
            try
            {
                return DivisionCodeData[region_code];
            }
            catch (Exception)
            {
